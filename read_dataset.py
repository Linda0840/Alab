...
df_iovation = pd.read_csv("hashed_iovation_reports.csv.gz", sep="|", engine="python")
df_iovation = df_iovation.dropna(subset=['CREDIT_REPORT'])
# Filter rows that start with '{' to keep only JSON-like entries
df_iovation_json = df_iovation[df_iovation['CREDIT_REPORT'].str.strip().str.startswith('{')]
...
# Apply to all rows
df_features = df_iovation_json['CREDIT_REPORT'].apply(parse_credit_report)

# df_features is now a Series of dicts
# Convert to DataFrame (keys become columns)
df_out = pd.DataFrame(df_features.tolist())

hashed_iovation_reports = pd.concat(
    [df_iovation_json.reset_index(drop=True),
     df_out.iloc[:, 0:45].reset_index(drop=True)],
    axis=1
)
